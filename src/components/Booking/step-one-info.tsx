...
interface Step1UserInfoProps {
  formData: UserInfoFormData
  onNext: (data: UserInfoFormData) => Promise<void>
}
...
export default function Step1UserInfo({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const parsed = userInfoSchema.safeParse(localData)
    if (!parsed.success) {
      const newErrors: Partial<Record<keyof UserInfoFormData, string>> = {}
    ...
    try {
      ...
      // Parent moves to step 2 — no setIsSubmitting(false) needed here
    } catch (error: any) {
  ...
  const inputClass = (field: keyof UserInfoFormData) =>
    ...
    <div className='mx-auto max-w-2xl'>
      ...
      <form onSubmit={handleSubmit} className='space-y-6'>
        <div className='rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900 sm:p-8'>
          ...
          {apiError && (
            <div className='mb-6 flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800 dark:border-red-800/50 dark:bg-red-900/20 dark:text-red-400'>
